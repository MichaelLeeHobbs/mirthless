...
> Last updated: 2026-03-01 (Phase 13)
...
| `@mirthless/engine` | Pipeline + Alerts | **Sandbox executor** (vm-based), **script compiler** (esbuild), **8-stage message pipeline**, **channel runtime** (state machine), **queue consumer** (with DB content loading), in-memory message store, **filter/transformer compilation**, **code template injection**, **global scripts**, **HL7 bridge functions**, **globalChannelMap**, **destinationSet**, **alert evaluator** (trigger matching), **action dispatcher** (LOG/CHANNEL actions), **alert manager** (throttle/max-alerts), **182 engine tests** | Persistent message store, wire alert manager into engine manager |
...
| `@mirthless/server` | API Phase 13 | Express app, config, middleware, DB schema, auth, seeds, **Channel CRUD API** (6 endpoints), **Deployment API** (8 endpoints), **Message Query API** (4 endpoints), **Statistics API** (3 endpoints), **User Management API** (7 endpoints), **Code Templates API** (8 endpoints), **Global Scripts API** (2 endpoints), **Alerts API** (6 endpoints), **Events API** (3 endpoints), **Settings API** (5 endpoints), **Data Pruner API** (3 endpoints), **Channel Export/Import API** (3 endpoints), **event emission** from all services, filter/transformer CRUD (inline with channel), **partition manager**, **data pruner**, **queue manager**, **279 server tests** | Wire alert evaluation into deployment |
| `@mirthless/web` | UI Phase 13 | React+MUI shell, auth flow, login page, **Channel Editor** (all 5 tabs + filter/transformer UI + **8 connector type forms**: TCP/MLLP, HTTP, File, Database, JavaScript, SMTP, Channel, FHIR), **Dashboard**, **Message Browser**, **Users Page**, **Code Templates Page**, **Global Scripts Page**, **Alerts Page**, **Events Page**, **Settings Page**, **Channel Export/Import** (ExportButton + ImportDialog), Monaco editor | DICOM connector UI |
...
| `pnpm test` | PASS | **995 tests passing** (184 schema + 68 HL7 + 182 engine + 282 connectors + 279 server) |

...
D-156: Real-DB integration tests are a separate, gated Vitest lane (`.itest.ts` + `vitest.integration.config.ts`, `pnpm test:integration`), NOT part of `pnpm test` — they run only when `DATABASE_URL` targets a `*_test` database (protects dev/prod DBs from destructive inserts; skips gracefully with no DB), and use the real `db` singleton (no mocks) to catch raw-SQL/schema drift that mock-DB unit tests can't. The `.itest.ts` suffix keeps them out of the default `*.test.ts` include, and no `test/setup.ts` is loaded so the real env `DATABASE_URL` reaches the singleton. — 2026-07-12

D-157: Data-pruner delete is broken and must be fixed before v0.1 (release blocker) — `packages/server/src/services/message-delete-helper.ts` builds `message_id = ANY(${ids})`; drizzle renders the JS array as invalid SQL and Postgres throws "malformed array literal", so `pruneChannel` always fails (surfaced fail-safe as a Failure Result; mocks hid it). Fix: use drizzle `inArray(table.col, ids)` or `= ANY(${ids}::bigint[])`. Integration test `data-pruner.itest.ts` currently asserts the fail-safe/atomic (no partial-delete) behavior and must be flipped to assert success once fixed. — 2026-07-12

D-158: Message-table partitioning is declared in the PartitionManagerService but NOT in the migrations (latent bug) — the six parent tables (messages, connector_messages, message_content, ...) ship as plain `pgTable`s, not `PARTITION BY LIST (channel_id)`, so `createPartitions()` fails against the real schema (ChannelService.create swallows it as a non-fatal warning; inserts still work because the tables are plain). Either add partitioning to the migrations or remove the partition manager. Confirmed by `partition-manager.itest.ts`, whose cross-check asserts createPartitions↔partitionExists consistency in either state. — 2026-07-12

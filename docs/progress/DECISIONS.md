...
D-167: SFTP connector added (source reader + destination writer) using `ssh2-sftp-client`, abstracted behind an injectable `SftpClientFactory` → `SftpClient` interface (mirrors SMTP `SmtpTransport` / Email `ImapClient` / DICOM factory DI) so both are unit-tested with a stateful in-memory mock — no real SFTP server. Config keys are the fixed contract the web forms bind to (source: host, port=22, username, password?, privateKey?, passphrase?, remoteDirectory, filePattern=`*`, pollingIntervalMs=5000, afterProcessing=DELETE|MOVE|NONE, moveToDirectory?, minFileAgeMs=1000, strictHostKey=false, hostKey?; dest: host, port=22, username, password?, privateKey?, passphrase?, remoteDirectory, fileNameTemplate=`${messageId}.dat`, appendMode=false, strictHostKey=false, hostKey?). — 2026-07-12

D-168: SFTP durable quarantine ledger lives as a REMOTE sidecar (`.mirthless-quarantine.json`) in `remoteDirectory`, not a local file — the File connector persists its ledger in the watched directory, and the SFTP equivalent of "the watched directory" is the remote dir. Keeps the whole quarantine lifecycle inside the `SftpClient` abstraction (fully mockable) and makes it survive a restart the same way (a new receiver downloads the ledger on start). Ledger is excluded from polling and keyed by `name:modifyTime` so an already-dispatched-but-post-action-failed file is never re-dispatched. — 2026-07-12

D-169: SFTP host-key check is lenient by default (`strictHostKey=false` → no `hostVerifier`, accept any key) because many internal healthcare SFTP hosts use self-signed/unmanaged keys; when `strictHostKey=true` an ssh2 `hostVerifier` accepts only a key matching the configured `hostKey` (base64 or raw), and rejects everything if `hostKey` is absent (fail closed). Credentials/keys are never logged. — 2026-07-12

D-170: SFTP dispatcher connects per-send and tears the connection down in a `finally` (mirrors SMTP dispatcher) rather than holding a long-lived connection — simplest correct behavior, avoids reusing a stale connection across queue retries, and honors the caller's AbortSignal + a 30s send timeout via `withTimeoutSignal`. Throughput is secondary to reliability for file drops. The receiver, by contrast, holds one connection and reconnects on poll-cycle failure (mirrors Email receiver). — 2026-07-12
D-171: Resolved D-166 with Option A (real partitioning), not removal. The PartitionManagerService code is already correct and well-tested — it only failed because the parent tables weren't partitioned. Chose to make the correct code work over deleting a deliberate design (PKs on all six tables already lead with `channel_id`, i.e. the schema was designed for LIST(channel_id) partitioning). Migration `0007_partition_message_tables.sql` drops and recreates `messages`, `connector_messages`, `message_content`, `message_custom_metadata`, `message_attachments`, `message_statistics` as `PARTITION BY LIST (channel_id)`, each with a DEFAULT partition (fail-safe: inserts for a channel whose per-channel partition wasn't created still land, rather than erroring — healthcare "never silently lose a message"). Postgres cannot ALTER a plain table to partitioned in place, so drop+recreate is required; safe because these tables hold no production data pre-release and have no FKs referencing them (no data-copy). Drizzle-kit cannot express partitioning, so the migration is hand-written and registered in `meta/_journal.json` (idx 7), following the existing hand-written-migration pattern (0003/0005 also have no meta snapshot). Verified against a throwaway Postgres 17: DDL applies; `createPartitions()`-equivalent `CREATE TABLE … PARTITION OF … FOR VALUES IN` succeeds; inserts route to the per-channel partition; inserts for a channel without a partition route to the DEFAULT partition; `dropPartitions()` + `partitionExists()` behave. `db:migrate` was NOT run against any live DB. — 2026-07-12

D-172: Per-destination resend re-enqueues rather than re-injecting the whole message. The engine's public surface (`getEngine().getRuntime()` → `DeployedChannel`) exposes no way to redispatch a single destination with stored content (the `sendFn` closure and the `DestinationConnectorRuntime` map are private to `EngineManager.deploy()`, and `ChannelRuntime.config` is private with destinations typed as lifecycle-only `RuntimeConnector`), and engine internals are owned by another wave. `MessageReprocessService.resendDestination` therefore flips the one connector message back to `QUEUED` (attempts reset) so the channel's already-running `QueueConsumer` redispatches the stored SENT content — a true, side-effect-isolated resend of exactly that destination (no duplicate sends to healthy destinations). It requires a queue-enabled destination (queueMode ≠ NEVER); a NEVER-queue destination has no consumer, so re-enqueuing would strand the message and is rejected with a clear 409 pointing to whole-message reprocess. This was chosen over a whole-message reprocess fallback (which would re-send to ALL destinations — a data-integrity hazard and a lying `/connectors/:metaDataId/resend` endpoint) and over a 501 stub. Resend is asynchronous (next consumer poll), so the response reports `status: 'QUEUED'`, not a synchronous send result. — 2026-07-12
D-173: Web response-transformer round-trips via the destination object, not the transformers[] array — the engine models a destination's response transformer as a single compiled script (`DestinationScripts.responseTransformer`), whereas `transformers[]` entries are steps-based and shared source/dest. So the web `ResponseTransformerSection` is a single-script Monaco editor and `ChannelEditorPage` emits `destinations[].responseTransformer: string | null` (read back from the same field). core-models `destinationInputSchema` does not yet carry this field (Zod strips it silently until added), and `useChannel`'s destination detail reads it defensively (`?? ''`). SERVER CONTRACT TO CONFIRM: server must (1) accept `responseTransformer` on `destinationInputSchema`, (2) project it on the destination detail response, and (3) compile it into `DestinationScripts.responseTransformer`. — 2026-07-12 (branch w2/web)

D-174: SFTP connector property keys are pinned in web defaults + a guard test — `connector-defaults.test.ts` asserts the exact source/dest key sets so the forms can't drift from what the (parallel) SFTP runtime connector reads. Source: host/port/username/password/privateKey/passphrase/remoteDirectory/filePattern/pollingIntervalMs/afterProcessing/moveToDirectory/minFileAgeMs/strictHostKey/hostKey. Dest: host/port/username/password/privateKey/passphrase/remoteDirectory/fileNameTemplate/appendMode/strictHostKey/hostKey. — 2026-07-12 (branch w2/web)

D-175: Forced password change is a modal gate, not a route — `mustChangePassword` (from the login response, persisted in auth.store) mounts a non-dismissable `ChangePasswordDialog forced` in `AppLayout`; the MUI modal traps focus/blocks interaction until the self-change endpoint succeeds, at which point the store flag is cleared. Chosen over a dedicated /change-password route to avoid touching the router and to keep the block in one place. — 2026-07-12 (branch w2/web)

D-176: Bulk message actions consume `messages:reprocess`/`messages:delete`; per the task these differ from the existing single-message header buttons which still gate on `channels:deploy`/`channels:delete`. Cross-channel `/messages` was intentionally left without bulk ops/export/resend — those endpoints are per-channel (require a channelId), so they don't map onto the cross-channel search surface. — 2026-07-12 (branch w2/web)

D-177: Collections — a durable, queryable, TTL-pruned keyed record store for channel scripts (new feature; design in `docs/design/10-collections.md`). Motivated by the user's live Mirth prod order/report matching (~100K/day, `portalApi.hl7message.store/find` + `populateReportMsg.getOrder`). Decisions: (1) SEPARATE from Resources — resources are static config blobs (read-only, cache-until-invalidated, `getResource`); collections are mutable high-churn keyed state (`getCollection`). Distinct tables/services/routes/bridges/pages. (2) APPEND-ONLY, not upsert — `store()` always inserts; many records accrue per key; "newest wins" is a query result (`ORDER BY created_at DESC LIMIT 1`), which matches the domain (orders accrue per accession over time) and avoids read-modify-write races at high write volume. (3) USER-DEFINABLE indexed fields (not a fixed accession/institution key) — a collection declares `indexedFields`; `find(match, {filter, latest})` matches equality on any subset (`fields @> match`, GIN-indexed) plus multiple AND'd filter fields (scalar=eq, array=IN). Bounded query surface, deliberately NOT a query language (no joins/ranges/partial keys in v1). (4) SINGLE static JSONB table `collection_records` (fields jsonb + payload + expire_at + created_at) — no per-collection dynamic DDL, so all queries stay parameterized/Drizzle-native. (5) TTL is a collection-level `defaultTtlSeconds` default with an optional per-write `expireAt` override (mirrors prod: backend default, rare per-customer override), pruned by the existing data-pruner scheduler. (6) NO read cache (unlike getResource) since collections are mutable. (7) Bridge injected at the same unpassed-deps point as the other IO bridges (`engine.ts` `new VmSandboxExecutor(...)`); API shaped like `portalApi.hl7message` for mechanical migration of existing channel code. — 2026-07-13 (branch feature/collections)
...
D-182: Connector-parity program (doc 12) execution decisions. Working through the 12 ranked gaps in `docs/design/12-connector-parity-gap-analysis.md` in rank order, each fully tested as its own PR. Owner calls on the §8 open decisions: (1) DB DRIVERS — the multi-vendor Database connector (rank 1) bundles pg + mysql2 + tedious by default; oracledb is an OPTIONAL peer install (heavy native dep + Oracle Instant Client), loaded lazily so its absence never breaks a non-Oracle install. (2) SFTP FATE — fold SFTP into the new File transport-scheme abstraction (rank 2) alongside local/FTP/FTPS/SMB/S3; keep the standalone SFTP connector as a thin alias over the scheme with a migration shim, and deprecate the standalone config shape (stops the per-transport-connector proliferation; SFTP was the first wrong step). (3) ROW FORMAT — non-Postgres Database sources emit JSON ONLY (the same row shape as Postgres); we do NOT implement Mirth's `<results>` XML output even as an opt-in (owner overrode the doc's XML-opt-in recommendation). MIGRATION NOTE: Mirth Database Reader channels whose downstream steps parse the `<results>` XML must rewrite those transformers to consume JSON. (4) JMS (rank 11) — DEFERRED until a concrete customer signal; not built in this program. — 2026-07-15 (branch feature/parity-gaps)

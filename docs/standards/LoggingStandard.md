...
| **debug** | Diagnostic detail for development. | Query parameters, cache hit/miss, routing decisions, pipeline timing |
...
Every operation that spans multiple steps or systems **shall** carry a correlation ID. This system uses two correlation scopes:

### 5.1 HTTP Request Correlation (`requestId`)

Tracks a single HTTP request/response cycle through the API layer.

```typescript
// Accept or generate a request ID at the boundary
...
- Generated by Express middleware for every API request
- Scoped to a single HTTP call — does not cross channel or protocol boundaries
- Returned in response headers (`X-Request-ID`)
- Used for: API debugging, rate limit tracing, error responses

### 5.2 Message Correlation (`correlationId`)

Tracks a message's full lifecycle across channels, protocols, and systems. This is the primary tracing mechanism for the message pipeline.

```typescript
// Generated when a message first enters the system (any source connector)
const correlationId = crypto.randomUUID();

// Carried through all pipeline stages
logger.debug({ correlationId, channelId, messageId, stage: 'sourceTransformer' }, 'Pipeline stage');

// Passed to destination connectors, preserved across channel-to-channel routing
dispatcher.send({ content, correlationId, sourceMap });

// Stored on the message record for query-time tracing
// SELECT * FROM messages WHERE correlation_id = 'abc-123'
```

**Rules:**
- Generated **once** at the point of first entry (source connector receives inbound message, or manual send)
- Propagated through: pipeline stages, destination sends, channel-to-channel routing
- Stored on the `messages` table (`correlation_id UUID`)
- When Channel A routes to Channel B, both messages share the same `correlationId`
- On **reprocess**: generate a new `correlationId`, but the `sourceMap` retains `originalCorrelationId` for linkage
- One `correlationId` → one grep → full message journey across all channels

### 5.3 Protocol-Specific Identifiers

Log both the internal `correlationId` and the protocol-native identifier so you can trace from either direction:

| Protocol | Native ID | Log Field |
|----------|-----------|-----------|
| HL7v2 | `MSH.10` (Message Control ID) | `hl7ControlId` |
| DICOM | SOP Instance UID | `sopInstanceUID` |
| HTTP | `X-Correlation-ID` header | `externalCorrelationId` |
| FHIR | `Bundle.id` / `Resource.id` | `fhirResourceId` |
| File | Filename | `sourceFileName` |
| Channel | Source channel + message ID | `sourceChannelId`, `sourceMessageId` |

```typescript
// HL7 message arrives via TCP/MLLP
log.info({
  correlationId,
  channelId,
  messageId,
  hl7ControlId: msg.get('MSH.10'),
  remoteAddress: sourceMap.remoteAddress,
}, 'Message received');
```

### 5.4 Typed Correlation Groups (recommended)
...
interface AuditEvent {
  ...
  readonly requestId?: string;  // HTTP correlation ID
...
| `LOG_HTTP_HEADERS` | Include HTTP headers in request logs | `true`, `false` |
...
correlationId across channels         Rely on messageId for cross-channel tracing
requestId for HTTP, correlationId     One ID for everything
  for messages
errMsg + stack for errors             Raw error objects
